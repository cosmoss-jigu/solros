...
kdb_per_cpu(int argc, const char **argv)
{
	...
	kdb_symtab_t symtab;
	kdb_symtab_t sympercpuend;
	cpumask_t suppress;
	int cpu, diag;
	unsigned long addr, val, byteoffset = 0, bytesperword = 0, whichcpu = ~0UL;
...
#else
	if (!kdbgetsymval(argv[1], &symtab)) {
		kdb_printf("%s is not a per_cpu variable\n", argv[1]);
		return KDB_BADADDR;
	}
	if (!kdbgetsymval("__per_cpu_end", &sympercpuend) &&
	    symtab.sym_start < sympercpuend.sym_start) {
		kdb_printf("%s is not a per_cpu variable\n", argv[1]);
		return KDB_BADADDR;
	}
#endif
	if (argc >=2 && (diag = kdbgetularg(argv[2], &byteoffset)))
		return diag;
	if (argc >=3 && (diag = kdbgetularg(argv[3], &bytesperword)))
		return diag;
	if (!bytesperword)
		bytesperword = KDB_WORD_SIZE;
	...
	if (argc >= 4) {
		if ((diag = kdbgetularg(argv[4], &whichcpu)))
			return diag;
...
#else
		addr = symtab.sym_start + byteoffset + per_cpu_offset(cpu);
#endif
