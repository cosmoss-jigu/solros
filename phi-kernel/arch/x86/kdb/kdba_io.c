...
#if defined(CONFIG_VT_CONSOLE) && !(defined(CONFIG_ML1OM) || defined(CONFIG_MK1OM))
...
struct uart_port kdb_uart_port;
struct uart_8250_port *kdb_serial_port;
...
serial_inp(struct kdb_serial *kdb_serial, unsigned long offset)
{
  unsigned int ret;
	...
	ret = (kdb_uart_port.serial_in)(kdb_serial_port, offset);
	return ret;
...
static struct tty_driver *kdb_tty_driver = NULL;
static int kdb_tty_line = 0;
