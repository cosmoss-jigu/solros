...
#define P9_PORT           564
#define MAX_SOCK_BUF      P9_ZC_DEFAULT_MSIZE /* (64*1024) */
...
static int p9_fd_get_pinned_pages(struct page **pages, char *data,
...
static int p9_fd_zc_write_io(struct p9_client *client, struct p9_req_t *req,
			     __u64 rg_addr, __u32 rg_len)
{
	int rc = p9pdu_writef(req->tc, client->proto_version,
			      "qd", rg_addr, rg_len);
	p9_debug(P9_DEBUG_TRANS, ": rg_addr: 0x%llx  len: %d  rc: %d\n",
		 rg_addr, rg_len, rc);
	return rc;
}


static int p9_fd_zc_write_iov(struct p9_client *client, struct p9_req_t *req,
			      ...
			      char *udata, int ulen)
{
	...
	for (i = 0; i < nr_pages; ++i) {
		...
		if (pg_addr != (prev_pg_addr + PAGE_SIZE)) {
			err = p9_fd_zc_write_io(client, req, rg_addr, rg_len);
			if (unlikely(err))
				goto err_out;
...
static int
p9_fd_zc_rsize(struct p9_client *client, u32 count)
{
	...
	n_iov = (client->msize - P9_IOHDRSZ) / iov_size;
...
/**
 * p9_fd_zc_request - issue a zero copy request
 ...
 */
static int
p9_fd_zc_request(struct p9_client *client, struct p9_req_t *req,
		 ...
		 int outlen, int in_hdr_len, int kern_buf)
{
	...
	p9_debug(P9_DEBUG_TRANS, "mux %p task %p tcall %p id %d udata %p ulen %d\n",
		 m, current, req->tc, req->tc->id, udata, ulen);
	...
	nr_pinned_pages = p9_fd_get_pinned_pages(pages, udata, nr_pages,
						 !write, kern_buf);
	...
	/* generate io vector */
	err = p9_fd_zc_write_iov(client, req, pages, nr_pages, udata, ulen);
	if (unlikely(err))
		goto unpin_out;

	/* send a request */
	err = p9_fd_request(client, req);
...
static int
p9_fd_zc_bulk_write_iov(struct p9_client *client, struct p9_req_t *req,
...
static int
p9_fd_zc_bulk_request(struct p9_client *client, struct p9_req_t *req,
                      ...
                      int len, int in_hdr_len)
{
	...
	p9_debug(P9_DEBUG_TRANS, "mux %p task %p tcall %p id %d len %d\n",
		 m, current, req->tc, req->tc->id, len);

	p9_debug(P9_DEBUG_TRANS,
		 ":%d %d pages are pinned\n",
		 __LINE__, pgcount);

	err = p9_fd_zc_bulk_write_iov(client, req, pages, pgcount, len);
	if (unlikely(err))
		goto err_out;
...
static struct p9_trans_module p9_tcp_trans = {
	...
	.zc_rsize = p9_fd_zc_rsize,
	.zc_request = p9_fd_zc_request,
#ifdef CONFIG_9P_FS_BULK_PAGE_IO
	.zc_bulk_request = p9_fd_zc_bulk_request,
#endif
#endif
	.cancel = p9_fd_cancel,
	.owner = THIS_MODULE,
