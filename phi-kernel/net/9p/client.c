...
static int parse_opts(char *opts, struct p9_client *clnt)
{
	...
	clnt->msize = P9_ZC_DEFAULT_MSIZE;
...
/**
 * p9_check_virtio_zc_errors - check 9p packet for error return and process it
 * @c: current client instance
 * @req: request to parse and check for error conditions
...
static int p9_check_virtio_zc_errors(struct p9_client *c, struct p9_req_t *req,
...
/**
 * p9_client_fd_zc_rpc - issue a request and wait for a response
 * @c: client session
 * @type: type of request
 ...
 * Returns request structure (which client must free using p9_free_req)
 */
static struct p9_req_t *__p9_client_fd_zc_rpc(struct p9_client *c, int8_t type,
...
static struct p9_req_t *
__p9_client_fd_zc_bulk_rpc(struct p9_client *c,
			   int8_t type, struct page **pages, u32 pgcount,
			   int len, int in_hdrlen, const char *fmt, ...)
{
	...
	err = c->trans_mod->zc_bulk_request(c, req, pages, pgcount,
					    len, in_hdrlen);
	if (err < 0) {
		if (err != -ERESTARTSYS && err != -EFAULT)
			c->status = Disconnected;
...
/**
 * p9_client_virtio_zc_rpc - issue a request and wait for a response
 * @c: client session
 * @type: type of request
 * @uidata: user bffer that should be ued for zero copy read
 * @uodata: user buffer that shoud be user for zero copy write
 ...
 * Returns request structure (which client must free using p9_free_req)
 */
static struct p9_req_t *__p9_client_virtio_zc_rpc(struct p9_client *c, int8_t type,
						  char *uidata, char *uodata,
						  int inlen, int olen, int in_hdrlen,
						  int kern_buf, const char *fmt, ...)
{
	...
	err = p9_check_virtio_zc_errors(c, req, uidata, in_hdrlen, kern_buf);
	trace_9p_client_res(c, type, req->rc->tag, err);
	if (!err)
		return req;
reterr:
...
struct p9_client *p9_client_create(const char *dev_name, char *options)
{
	...
	if (clnt->trans_mod == v9fs_get_trans_by_name("virtio")) {
		clnt->zc_rpc = __p9_client_virtio_zc_rpc;
#ifdef CONFIG_9P_FS_BULK_PAGE_IO
		clnt->zc_bulk_rpc = NULL;
#endif
    }
	else {
		clnt->zc_rpc = __p9_client_fd_zc_rpc;
#ifdef CONFIG_9P_FS_BULK_PAGE_IO
		clnt->zc_bulk_rpc = __p9_client_fd_zc_bulk_rpc;
#endif
    }
...
int
p9_client_rdwr_size(struct p9_fid *fid, u32 count)
{
	struct p9_client *clnt = fid->clnt;
	int rsize;

	if (clnt->trans_mod->zc_rsize)
		rsize = clnt->trans_mod->zc_rsize(clnt, count);
	else {
		rsize = fid->iounit;
...
EXPORT_SYMBOL(p9_client_rdwr_size);
								...
								u32 count)
{
	...
	p9_debug(P9_DEBUG_9P, ">>> TREAD fid %d offset %llu count %d\n",
		   fid->fid, (unsigned long long) offset, count);
	err = 0;
	clnt = fid->clnt;
	rsize = p9_client_rdwr_size(fid, count);

	p9_debug(P9_DEBUG_9P, ": data: %p  udata: %p  offset: %lld"
		              "  count: %d  rsize: %d\n",
		 data, udata, offset, count, rsize);

	/* Don't bother zerocopy for small IO */
	if (clnt->trans_mod->zc_request && rsize > 1024) {
		 ...
		 */
		req = clnt->zc_rpc(clnt, P9_TREAD, indata, NULL, rsize, 0,
...
#ifdef CONFIG_9P_FS_BULK_PAGE_IO
int
p9_client_bulk_read(struct p9_fid *fid, struct page **pages,
                    int pgcount, int p9_size, loff_t offset)
{
	struct p9_client *clnt;
	...
	p9_debug(P9_DEBUG_9P, ">>> TREAD fid %d offset %llu count %d\n",
		 fid->fid, (unsigned long long) offset, p9_size);
	BUG_ON(p9_size % PAGE_SIZE);

	pgcount = p9_size / PAGE_SIZE;
	count = (u32)p9_size;
	err = 0;
	clnt = fid->clnt;
	 ...
	 */
	req = clnt->zc_bulk_rpc(clnt, P9_TREAD, pages, pgcount,
				p9_size, 11, "dqd",
				fid->fid, offset, p9_size);
	if (IS_ERR(req)) {
		err = PTR_ERR(req);
		goto error;
	}
...
EXPORT_SYMBOL(p9_client_bulk_read);

int
p9_client_bulk_write(struct p9_fid *fid, struct page **pages,
                    int pgcount, int p9_size, loff_t offset)
{
	struct p9_client *clnt;
	struct p9_req_t *req = NULL;
	 ...
	 * it is a kernel buffer
	 */
	req = clnt->zc_bulk_rpc(clnt, P9_TWRITE, pages, pgcount,
				p9_size, 11, "dqd",
				fid->fid, offset, p9_size);
	if (IS_ERR(req)) {
		err = PTR_ERR(req);
...
EXPORT_SYMBOL(p9_client_bulk_write);
#endif
							...
							u64 offset, u32 count)
{
	...
	if (clnt->trans_mod->zc_request && rsize > 1024) {
		...
		if (data) {
			...
			odata = (char *)udata;
		req = clnt->zc_rpc(clnt, P9_TWRITE, NULL, odata, 0, rsize,
