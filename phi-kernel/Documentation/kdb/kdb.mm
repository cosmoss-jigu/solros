...
.TP 8
per_cpu <variable_name> <offset> <length> [<cpu>]
Display the values of a per_cpu variable.  Offset is the field offset, obtained with objdump or pahole.
Length is the length of the variable, 1, 2, 4 or 8 bytes.
To display the variable on a specific cpu, the forth parameter is the
cpu number.
When the forth parameter is omitted, the variable's value is printed
