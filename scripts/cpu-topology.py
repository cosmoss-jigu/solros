...
def print_cpu_topology_in_python(cpuinfo, topology_map, socket_id_set, physical_cpu_id_set):
    ...
    for (s, socket_id) in enumerate(sorted(socket_id_set)):
        for (p, physical_cpu_id) in enumerate(sorted(physical_cpu_id_set)):
            key = ":".join([str(socket_id), str(physical_cpu_id)])
            for (smt_id, os_cpu_id) in enumerate(topology_map[key]):
                print("OS_CPU_ID[%s,%s,%s] = %s   # %s, %s, %s" %
                      (s, p, smt_id, os_cpu_id,
                       socket_id, physical_cpu_id, smt_id))
    print("")
