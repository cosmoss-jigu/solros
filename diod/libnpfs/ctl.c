...
{
	Fid *f = fid->aux;
	Npfcall *tc = req->tcall, *rc = NULL;
	int len;

	if ((f->file->flags & NP_CTL_FLAGS_DELAY100MS))
		usleep (100*1000);
	if ((f->file->flags & NP_CTL_FLAGS_ZEROSRC) && !tc->u.tread.n_iov) {
		if ((rc = np_alloc_rread (tc->u.tread.n_iov, count)))
	...
	if (count > len - offset)
		count = len - offset;
	if (!(rc = np_create_rread (count, tc->u.tread.n_iov,
				    (u8 *)f->data + offset))) {
		np_uerror (ENOMEM);
		goto done;
	}
