...
struct ioctx_struct {
    ...
    pthread_spinlock_t fblkmap_lock;
    int             refcount;
    int             fd;      /* fd for non-zero-copy operations */
    int             fd_grow; /* fd for zero-copy operations */
    DIR             *dir;
    int             lock_type;
    Npqid           qid;
    u32             iounit;
...
_ioctx_close_destroy (IOCtx ioctx, int seterrno)
{
    ...
    if (ioctx->dir) {
        ...
        if (rc < 0 && seterrno)
            np_uerror (errno);
    } else if (ioctx->fd_grow != -1) {
        rc = close (ioctx->fd_grow);
        if (rc < 0 && seterrno)
            np_uerror (errno);
    }
    if (ioctx->user)
        np_user_decref (ioctx->user);
    pthread_spin_destroy (&ioctx->fblkmap_lock);
    free(ioctx->fblkmap);
...
_ioctx_create_open (Npuser *user, Path path, int flags, u32 mode)
{
    ...
    pthread_spin_init (&ioctx->fblkmap_lock, PTHREAD_PROCESS_PRIVATE);
    ioctx->refcount = 1;
    ...
    ioctx->fd_grow = open (path->s, O_DIRECT | O_WRONLY, mode);
    /* error can be safely ignored here
     * because error will anyway happen
     * at actual pread/pwrite. */
...
int
ioctx_truncate(Npfid *fid, u64 size)
{
    Fid *f = fid->aux;
    int rc;
    ...
    pthread_spin_lock(&f->ioctx->fblkmap_lock); {
	    /* truncate a file */
	    rc = truncate (path_s (f->path), size);
	    if (!rc) {
		    /* then invalidate its file block mapping */
		    free(f->ioctx->fblkmap);
		    f->ioctx->fblkmap = NULL;
		    f->ioctx->n_fblkmap = 0;
	    }
    } pthread_spin_unlock(&f->ioctx->fblkmap_lock);
    return rc;
}
...
int
ioctx_pread_zc (IOCtx ioctx, int n_iov, struct p9_zc_io *iov,
		size_t count, off_t offset)
{
    return pread_zc (&ioctx->fblkmap_lock,
		     ioctx->fd, /* we use 'fd' since there is no way to expand. */
		     &ioctx->n_fblkmap, &ioctx->fblkmap,
		     n_iov, iov, count, offset);
}
...
int
ioctx_pwrite_zc (IOCtx ioctx, int n_iov, struct p9_zc_io *iov,
		 size_t count, off_t offset)
{
    return pwrite_zc (&ioctx->fblkmap_lock,
		      ioctx->fd_grow, /* we use 'fd_grow' since a file can grow. */
		      &ioctx->n_fblkmap, &ioctx->fblkmap,
		      n_iov, iov, count, offset);
}
