...
int     ioctx_truncate(Npfid *fid, u64 size);
int     ioctx_pread (IOCtx ioctx, void *buf, size_t count, off_t offset);
int     ioctx_pread_zc (IOCtx ioctx, int n_iov, struct p9_zc_io *iov,
			size_t count, off_t offset);
int     ioctx_pwrite (IOCtx ioctx, const void *buf, size_t count, off_t offset);
int     ioctx_pwrite_zc (IOCtx ioctx, int n_iov, struct p9_zc_io *iov,
			 size_t count, off_t offset);
