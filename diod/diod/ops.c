...
#define DIOD_SERVER_MSIZE (512 * 1024) /* 65536 */

int
diod_init (Npsrv *srv)
{
    srv->msize = DIOD_SERVER_MSIZE;
...
{
    Fid *f = fid->aux;
    Npfcall *tc = req->tcall, *ret = NULL;
    u32 n_iov = tc->u.tread.n_iov;
    struct p9_zc_io *iov = tc->u.tread.iov;
    ssize_t n;
    ...
    if (!(ret = np_alloc_rread (n_iov, count))) {
        np_uerror (ENOMEM);
        goto error;
    }
    if (!n_iov) {
	    ...
	    if (f->flags & DIOD_FID_FLAGS_XATTR)
		    n = xattr_pread_zc (f->xattr, n_iov, iov, count, offset);
	    else
		    n = ioctx_pread_zc (f->ioctx, n_iov, iov, count, offset);
    }
    if (n < 0) {
        np_uerror (errno);
        goto error;
    }
    np_set_rread_count (ret, n_iov, n);
    return ret;
error:
...
diod_write (Npfid *fid, u64 offset, u32 count, u8 *data, Npreq *req)
{
    Fid *f = fid->aux;
    Npfcall *tc = req->tcall, *ret = NULL;
    u32 n_iov = tc->u.twrite.n_iov;
    struct p9_zc_io *iov = tc->u.twrite.iov;
    ssize_t n;
    ...
    if (!n_iov) {
	    ...
	    if (f->flags & DIOD_FID_FLAGS_XATTR)
		    n = xattr_pwrite_zc (f->xattr, n_iov, iov, count, offset);
	    else
		    n = ioctx_pwrite_zc (f->ioctx, n_iov, iov, count, offset);
    }
    if (n < 0) {
        np_uerror (errno);
        goto error_quiet;
...
diod_lopen (Npfid *fid, u32 flags)
{
    ...
    if (flags & O_DIRECT && !is_zc_file(fid)) {
