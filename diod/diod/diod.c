...
#define OPTIONS "fr:w:d:l:t:e:Eo:u:SL:npc:NU:sv:m:"
...
static const struct option longopts[] = {
    ...
    {"nvme",            required_argument,  0, 'v'},
    {"mic",             required_argument,  0, 'm'},
    {0, 0, 0, 0},
};
#else
...
"   -v,--nvme              NCME device path for peer-to-peer with MIC\n"
"   -m,--mic               MIC ID for peer-to-peer with NVME\n"
    );
    exit (1);
}
    ...
    while ((c = GETOPT (argc, argv, OPTIONS, longopts)) != -1) {
        switch (c) {
            ...
            case 'L':   /* --logdest DEST */
                ...
                break;
            case 'v':   /* --nvme */
                diod_conf_set_dev_nvme (optarg);
                break;
            case 'm':   /* --mic */
	        diod_conf_set_mic_id ( atoi(optarg));
                break;
            default:
                usage();
        }
...
#if WITH_PRBTRANS
static void *
_service_loop_prb (void *arg)
{
    while (!ss.shutdown) {
        msg ("waiting on prb connection");
        diod_prb_accept_one (ss.srv, ss.prb);
    }
    return NULL;
}
#endif
...
#endif
#if WITH_PRBTRANS
            ss.prb = diod_prb_create ();
            diod_prb_listen (l, ss.prb);
#endif
            break;
    }
...
#endif
#if WITH_PRBTRANS
    if ((n = pthread_create (&ss.prb_t, NULL, _service_loop_prb, NULL)))
        errn_exit (n, "pthread_create _service_loop_prb");
#endif
    switch (mode) {
        case SRV_FILEDES:
        case SRV_SOCKTEST:
...
#endif
#if WITH_PRBTRANS
    if ((n = pthread_join (ss.prb_t, NULL)))
        errn_exit (n, "pthread_join _service_loop_prb");
#endif
