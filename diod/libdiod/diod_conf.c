...
diod_conf_init (void)
{
    ...
    config.ro_mask = 0;
    config.dev_nvme = _xstrdup(DFLT_DEV_NVME);
    config.mic_id = DFLT_MIC_ID;
}
...
/* nvme-to-mic peer-to-peer
 */
char   *diod_conf_get_dev_nvme(void)
{
    return config.dev_nvme;
}

void    diod_conf_set_dev_nvme(char *dev_nvme)
{
    free(config.dev_nvme);
    config.dev_nvme = _xstrdup(dev_nvme);
}

int     diod_conf_get_mic_id(void)
{
    return config.mic_id;
}

void    diod_conf_set_mic_id(int mic_id)
{
    config.mic_id = mic_id;
}
