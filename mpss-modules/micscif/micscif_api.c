...
int scif_mic_dma_info(uint16_t node, struct mic_dma_info *mic_dma_info)
{
	mic_ctx_t *mic_ctx = get_per_dev_ctx(node - 1);
	...
	/* get mic dma info */
	mic_dma_info->va  = mic_ctx->aper.va;
	mic_dma_info->pa  = mic_ctx->aper.pa;
	mic_dma_info->len = mic_ctx->aper.len;
	return 0;
}
EXPORT_SYMBOL(scif_mic_dma_info);
#endif
