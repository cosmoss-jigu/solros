...
#include <trace/events/micveth_dma.h>
				  ...
				  struct sched_node *snode)
{
	...
	trace_send_dma_complete(body->size);
...
skip_adding_new_buffers:
	...
	local_bh_disable();
	trace_recv_dma_complete(msg->size);
...
micvnet_init_netdev(struct micvnet_info *vnet_info)
{
	...
	if ((dev_vnet = (struct net_device *)alloc_netdev(sizeof(struct micvnet_info), 
							  "mic%d", 
#ifdef HOST
							  NET_NAME_UNKNOWN,
#endif
